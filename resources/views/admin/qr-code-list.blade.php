                    ...
                    <tbody>
                        @foreach ($applications as $application)
                            <tr>
                                ...
                                <td>{{ $application->length ."*". $application?->width." Ft." .' / '. $application?->price }}</td>
                                <td>{{ $application->advertise_detail }}</td>
                                <td>
                                    <a href="{{ asset($application->banner_image) }}" target="_blank" class="btn btn-info">View Banner</a>
                                </td>
                                <td>
                                    <a href="{{ route('download-qr-code', $application->id) }}" target="_blank"><button class="edit-element btn px-2 py-1" title="Download QR Code"><i class="fas fa-qrcode"></i> &nbsp;QR Code </button></a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
